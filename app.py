...
for item, price in all_menu.items():
    cursor.execute("INSERT OR IGNORE INTO inventory VALUES (?,?,?)",
                   (item, 50, price * 0.6))  # cost = 60% of price
conn.commit()
...
menu = st.sidebar.radio(
    ...
    ["Home", "Login", "Register", "Branch",
     "Menu List", "Order", "Cart",
     "Admin Dashboard"]
)
...
if menu == "Home":
    ...
    st.subheader("Enterprise French Bakery & Coffee System")
    st.write("Multi-Branch • Secure • Scalable • Profit-Tracked")
...
# ==========================================================
    ...
    if not st.session_state.cart:
        ...
        st.subheader("💳 GCash Payment Simulation")
        gcash = st.text_input("GCash Number")
        otp = st.text_input("OTP")

        if st.button("Confirm Payment"):
            if len(gcash) == 11 and len(otp) == 6:
                ...
                for item, qty, _ in st.session_state.cart:
                    cursor.execute("UPDATE inventory SET stock=stock-? WHERE item=?",
                                   (qty, item))
                conn.commit()
